...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("document_analysis_2.log"),
...
class LocalPDFAnalyzer:
    def __init__(self, model="gemma3:7b"):
        self.model = model
    ...
    def classify_document(self, text, exclusion_criteria=None, classification_type="exclusion"):
        if not text.strip():
        ...
        try:
            if classification_type == "exclusion":
                ...
                prompt = f"""
                You are reviewing medical document about prostate cancer that has been classified for EXCLUSION.
                ...
                2. Read through the document and explain why it meets the reason for exclusion
                3. Identify specific keywords or phrases that led to exclusion
                
                Provide your analysis in the following JSON format:
                {{
                  "classification": "The given exclusion reason",
                  "keywords": ["list", "of", "key", "terms", "from", "the", "document", "that", "justify", "exclusion"],
                ...
                Document text:
                "Este estudio analiza la progresión de tumores de próstata en pacientes tratados con terapia hormonal."

                Expected output:
                {{
                "classification": "Non-English",
                "keywords": ["progresión", "tumores de próstata", "terapia hormonal"],
                "reason": "The article is written in Spanish, which does not meet the inclusion criteria for English-language studies."
                }}
                ----
                ...
                prompt = f"""
                You are reviewing medical documents about prostate cancer that has been classified for INCLUSION.
                Your task is to analyze this document and explain WHY it should be included for prostate cancer research.
                ...
                {{
                  ...
                  "keywords": ["list", "of", "key", "terms", "from", "the", "document", "that", "justify", "inclusion"],
                ...
                Document text:
                "This study investigates the effects of androgen receptor inhibitors in patients with metastatic castration-resistant prostate cancer (mCRPC). Data from 250 patients were analyzed to evaluate the treatment's efficacy and progression-free survival rates."

                Expected output:
                {{
                "classification": "Study on prostate cancer treatment",
                "keywords": ["androgen receptor inhibitors", "metastatic castration-resistant prostate cancer", "mCRPC", "treatment efficacy", "progression-free survival"],
                "reason": "The document presents a clinical study focused on treatment strategies for metastatic prostate cancer, specifically mCRPC, which is directly relevant to prostate cancer research."
                }}
                ----
    ...
    def process_pdf_files(self, exclusion_pdfs, inclusion_pdfs):
        ...
        for pdf_path in tqdm(exclusion_pdfs):
            ...
            # Extract exclusion criteria from filename
            exclusion_criteria = pdf_path.split('(')[1].split(')')[0]
            exclusion_criteria = exclusion_criteria.replace("_", " / ")
            ...
            if text.strip():
                ...
                # Get classification
                classification = self.classify_document(text, exclusion_criteria, "exclusion")
                result = {
        ...
        for pdf_path in tqdm(inclusion_pdfs):
            ...
            if text.strip():
                ...
                # Get classification
                classification = self.classify_document(text, classification_type="inclusion")
                result = {
                    "file": os.path.basename(pdf_path),
                    "classification": classification
                }
